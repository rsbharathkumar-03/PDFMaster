...
* Convert an internal view ID into a SEO-friendly URL.
*
* Examples:
* home           -> /
* merge-pdf      -> /merge-pdf
...
const matchingTool = toolsCatalog.find((tool) => tool.id === path);

if (matchingTool) {
return matchingTool.id as ViewType;
}
...
* Synchronize browser Back/Forward buttons with the application.
  ...
  *
  * We use replaceState instead of pushState here so that
  * loading the application does not create an unnecessary
  * history entry.
    */
...
const handlePopState = (event: PopStateEvent) => {
  if (event.state && event.state.view) {
    ...
    // If the browser doesn't have our state,
...
* Handle the custom floating Back button.
  */
  ...
  const previousPath = viewToPath(previousView);
...
? toolsCatalog.find((t) => t.id === previousViewKey)
: undefined;
...
return ( <div className="flex min-h-screen flex-col bg-slate-50 font-sans text-slate-900 antialiased selection:bg-indigo-500 selection:text-white relative">
...
```
  ...
  <Footer
    onNavigate={handleNavigate}
    onOpenBackendCode={() => setShowJavaModal(true)}
  />
